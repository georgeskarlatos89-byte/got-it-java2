...
public final class DebugUtils {
    ...
    public static String readableBufferInfo(MediaCodec.BufferInfo bufferInfo) {
        ...
        sb.append("Dump BufferInfo: " + bufferInfo.toString() + ReactEditTextInputConnectionWrapper.NEWLINE_RAW_VALUE);
        sb.append("\toffset: " + bufferInfo.offset + ReactEditTextInputConnectionWrapper.NEWLINE_RAW_VALUE);
        sb.append("\tsize: " + bufferInfo.size + ReactEditTextInputConnectionWrapper.NEWLINE_RAW_VALUE);
    ...
    private static String formatInterval(long j) {
        ...
        return String.format(Locale.US, "%02d:%02d:%02d.%03d", new Object[]{Long.valueOf(hours), Long.valueOf(minutes), Long.valueOf(seconds), Long.valueOf(millis)});
    }
    ...
    private static void dumpVideoCapabilities(StringBuilder sb, MediaCodecInfo.VideoCapabilities videoCapabilities, MediaFormat mediaFormat) {
        ...
        if (z) {
            ...
            logToString(sb, "[VideoCaps] isSizeSupported for " + i2 + "x" + i + " = " + videoCapabilities.isSizeSupported(i2, i));
        }
        ...
        if (z) {
            logToString(sb, "[VideoCaps] getSupportedFrameRatesFor " + i2 + "x" + i + " = " + videoCapabilities.getSupportedFrameRatesFor(i2, i));
        }
        if (z && i3 > 0) {
            logToString(sb, "[VideoCaps] areSizeAndRateSupported for " + i2 + "x" + i + ", " + i3 + " = " + videoCapabilities.areSizeAndRateSupported(i2, i, (double) i3));
        }
    }
