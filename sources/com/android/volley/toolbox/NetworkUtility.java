...
final class NetworkUtility {
    ...
    static void logSlowRequests(long j, Request<?> request, byte[] bArr, int i) {
        if (VolleyLog.DEBUG || j > 3000) {
            ...
            objArr[2] = bArr != null ? Integer.valueOf(bArr.length) : ThreeDSStrings.NULL_STRING;
    ...
    static NetworkResponse getNotModifiedNetworkResponse(Request<?> request, long j, List<Header> list) {
        ...
        if (cacheEntry == null) {
            return new NetworkResponse((int) HttpStatus.SC_NOT_MODIFIED, (byte[]) null, true, j, list);
        }
        return new NetworkResponse((int) HttpStatus.SC_NOT_MODIFIED, cacheEntry.data, true, j, HttpHeaderParser.combineHeaders(list, cacheEntry));
    }
