...
class AESCBC {
    ...
    public static AuthenticatedCipherText encryptWithConcatKDF(JWEHeader jWEHeader, SecretKey secretKey, Base64URL base64URL, byte[] bArr, byte[] bArr2, Provider provider, Provider provider2) throws JOSEException {
        ...
        return new AuthenticatedCipherText(encrypt, HMAC.compute(LegacyConcatKDF.generateCIK(secretKey, jWEHeader.getEncryptionMethod(), decode, bArr3), (String.valueOf(jWEHeader.toBase64URL().toString()) + "." + base64URL.toString() + "." + Base64URL.encode(bArr).toString() + "." + Base64URL.encode(encrypt)).getBytes(), provider2));
    }
