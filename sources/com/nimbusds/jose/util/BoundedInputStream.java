...
public class BoundedInputStream extends InputStream {
    ...
    public int read(byte[] bArr, int i, int i2) throws IOException {
        long j = this.max;
        if (j < 0 || this.pos < j) {
            int read = this.f10in.read(bArr, i, i2);
            if (read == -1) {
                return -1;
            }
    ...
    public int available() throws IOException {
        long j = this.max;
        if (j < 0 || this.pos < j) {
            return this.f10in.available();
        }
        return 0;
    }

    public String toString() {
        return this.f10in.toString();
    }
    ...
    public boolean markSupported() {
        return this.f10in.markSupported();
    }
