    ...
    public void onReceive(Context context, Intent intent) {
        if (!CrashShieldHandler.isObjectCrashing(this)) {
            try {
                ...
                if (set != null) {
                    for (String str2 : set) {
                        Intrinsics.checkNotNullExpressionValue(str2, SDKConstants.PARAM_KEY);
                        bundle.putString(new Regex("[ -]*$").replace((CharSequence) new Regex("^[ -]*").replace((CharSequence) new Regex("[^0-9a-zA-Z _-]").replace((CharSequence) str2, "-"), ""), ""), (String) bundleExtra.get(str2));
                    }
                }
