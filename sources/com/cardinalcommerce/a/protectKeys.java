package com.cardinalcommerce.a;

import java.math.BigInteger;

public final class protectKeys {
    private static final int[] Cardinal = {-1, -1, -1, 1, 0, 0, -1, -3, -1, -1, 1};
    static final int[] cca_continue = {1, 0, 0, -1, -1, -1, -1};
    private static int[] init = {1, 0, 0, -2, -1, -1, 0, 2, 0, 0, -2, -1, -1, -1};

    public static void Cardinal(int[] iArr, int[] iArr2, int[] iArr3) {
        if (getSDKTransactionID.init(iArr, iArr2, iArr3) != 0 || (iArr3[6] == -1 && getSDKTransactionID.Cardinal(iArr3, cca_continue))) {
            configure(iArr3);
        }
    }

    public static void cca_continue(int[] iArr, int[] iArr2) {
        if (getErrorMessage.configure(7, iArr, iArr2) != 0 || (iArr2[6] == -1 && getSDKTransactionID.Cardinal(iArr2, cca_continue))) {
            configure(iArr2);
        }
    }

    public static void getInstance(int[] iArr, int[] iArr2) {
        if (getErrorMessage.cca_continue(7, iArr, 0, iArr2) != 0 || (iArr2[6] == -1 && getSDKTransactionID.Cardinal(iArr2, cca_continue))) {
            configure(iArr2);
        }
    }

    public static int[] configure(BigInteger bigInteger) {
        int[] instance = getSDKTransactionID.getInstance(bigInteger);
        if (instance[6] == -1) {
            int[] iArr = cca_continue;
            if (getSDKTransactionID.Cardinal(instance, iArr)) {
                long j = ((((long) instance[0]) & 4294967295L) - (((long) iArr[0]) & 4294967295L)) + 0;
                instance[0] = (int) j;
                long j2 = (j >> 32) + ((((long) instance[1]) & 4294967295L) - (((long) iArr[1]) & 4294967295L));
                instance[1] = (int) j2;
                long j3 = (j2 >> 32) + ((((long) instance[2]) & 4294967295L) - (((long) iArr[2]) & 4294967295L));
                instance[2] = (int) j3;
                long j4 = (j3 >> 32) + ((((long) instance[3]) & 4294967295L) - (((long) iArr[3]) & 4294967295L));
                instance[3] = (int) j4;
                long j5 = (j4 >> 32) + ((((long) instance[4]) & 4294967295L) - (((long) iArr[4]) & 4294967295L));
                instance[4] = (int) j5;
                long j6 = (j5 >> 32) + ((((long) instance[5]) & 4294967295L) - (((long) iArr[5]) & 4294967295L));
                instance[5] = (int) j6;
                instance[6] = (int) ((j6 >> 32) + ((((long) instance[6]) & 4294967295L) - (((long) iArr[6]) & 4294967295L)));
            }
        }
        return instance;
    }

    public static void cca_continue(int[] iArr, int[] iArr2, int[] iArr3) {
        int[] iArr4 = new int[14];
        getSDKTransactionID.getInstance(iArr, iArr2, iArr4);
        init(iArr4, iArr3);
    }

    public static void getInstance(int[] iArr, int[] iArr2, int[] iArr3) {
        if (getSDKTransactionID.Cardinal(iArr, iArr2, iArr3) != 0 || (iArr3[13] == -1 && getErrorMessage.init(14, iArr3, init))) {
            int[] iArr4 = Cardinal;
            if (getErrorMessage.Cardinal(iArr4.length, iArr4, iArr3) != 0) {
                int length = iArr4.length;
                while (length < 14) {
                    int i = iArr3[length] + 1;
                    iArr3[length] = i;
                    if (i == 0) {
                        length++;
                    } else {
                        return;
                    }
                }
            }
        }
    }

    public static void Cardinal(int[] iArr, int[] iArr2) {
        boolean z;
        int i = 0;
        while (true) {
            if (i >= 7) {
                z = true;
                break;
            } else if (iArr[i] != 0) {
                z = false;
                break;
            } else {
                i++;
            }
        }
        if (z) {
            iArr2[0] = 0;
            iArr2[1] = 0;
            iArr2[2] = 0;
            iArr2[3] = 0;
            iArr2[4] = 0;
            iArr2[5] = 0;
            iArr2[6] = 0;
            return;
        }
        getSDKTransactionID.getSDKVersion(cca_continue, iArr, iArr2);
    }

    /* JADX WARNING: Code restructure failed: missing block: B:12:0x00e0, code lost:
        if (r11 == false) goto L_0x00e2;
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    public static void init(int[] r32, int[] r33) {
        /*
            r0 = r33
            r1 = 10
            r1 = r32[r1]
            long r1 = (long) r1
            r3 = 4294967295(0xffffffff, double:2.1219957905E-314)
            long r1 = r1 & r3
            r5 = 11
            r5 = r32[r5]
            long r5 = (long) r5
            long r5 = r5 & r3
            r7 = 12
            r7 = r32[r7]
            long r7 = (long) r7
            long r7 = r7 & r3
            r9 = 13
            r9 = r32[r9]
            long r9 = (long) r9
            long r9 = r9 & r3
            r11 = 7
            r12 = r32[r11]
            long r12 = (long) r12
            long r12 = r12 & r3
            long r12 = r12 + r5
            r14 = 1
            long r12 = r12 - r14
            r16 = 8
            r11 = r32[r16]
            long r14 = (long) r11
            long r14 = r14 & r3
            long r14 = r14 + r7
            r11 = 9
            r11 = r32[r11]
            r17 = r7
            long r7 = (long) r11
            long r7 = r7 & r3
            long r7 = r7 + r9
            r11 = 0
            r19 = r9
            r9 = r32[r11]
            long r9 = (long) r9
            long r9 = r9 & r3
            long r9 = r9 - r12
            r21 = 0
            long r9 = r9 + r21
            long r23 = r9 & r3
            r16 = 32
            long r9 = r9 >> r16
            r25 = 1
            r11 = r32[r25]
            r26 = r5
            long r5 = (long) r11
            long r5 = r5 & r3
            long r5 = r5 - r14
            long r9 = r9 + r5
            int r5 = (int) r9
            r0[r25] = r5
            long r9 = r9 >> r16
            r6 = 2
            r11 = r32[r6]
            r28 = r14
            long r14 = (long) r11
            long r14 = r14 & r3
            long r14 = r14 - r7
            long r9 = r9 + r14
            int r11 = (int) r9
            r0[r6] = r11
            long r9 = r9 >> r16
            r14 = 3
            r15 = r32[r14]
            long r14 = (long) r15
            long r14 = r14 & r3
            long r14 = r14 + r12
            long r14 = r14 - r1
            long r9 = r9 + r14
            long r12 = r9 & r3
            long r9 = r9 >> r16
            r14 = 4
            r15 = r32[r14]
            r30 = r7
            long r6 = (long) r15
            long r6 = r6 & r3
            long r6 = r6 + r28
            long r6 = r6 - r26
            long r9 = r9 + r6
            int r6 = (int) r9
            r0[r14] = r6
            long r6 = r9 >> r16
            r9 = 5
            r10 = r32[r9]
            long r14 = (long) r10
            long r14 = r14 & r3
            long r14 = r14 + r30
            long r14 = r14 - r17
            long r6 = r6 + r14
            int r10 = (int) r6
            r0[r9] = r10
            long r6 = r6 >> r16
            r9 = 6
            r10 = r32[r9]
            long r14 = (long) r10
            long r14 = r14 & r3
            long r14 = r14 + r1
            long r14 = r14 - r19
            long r6 = r6 + r14
            int r1 = (int) r6
            r0[r9] = r1
            long r1 = r6 >> r16
            r6 = 1
            long r1 = r1 + r6
            long r12 = r12 + r1
            long r1 = r23 - r1
            int r6 = (int) r1
            r7 = 0
            r0[r7] = r6
            long r1 = r1 >> r16
            int r6 = (r1 > r21 ? 1 : (r1 == r21 ? 0 : -1))
            if (r6 == 0) goto L_0x00c3
            long r5 = (long) r5
            long r5 = r5 & r3
            long r1 = r1 + r5
            int r5 = (int) r1
            r0[r25] = r5
            long r1 = r1 >> r16
            long r5 = (long) r11
            long r3 = r3 & r5
            long r1 = r1 + r3
            int r3 = (int) r1
            r4 = 2
            r0[r4] = r3
            long r1 = r1 >> r16
            long r12 = r12 + r1
        L_0x00c3:
            int r1 = (int) r12
            r2 = 3
            r0[r2] = r1
            long r1 = r12 >> r16
            int r1 = (r1 > r21 ? 1 : (r1 == r21 ? 0 : -1))
            if (r1 == 0) goto L_0x00e2
            r1 = 7
            r14 = 4
        L_0x00cf:
            if (r14 >= r1) goto L_0x00de
            r2 = r0[r14]
            int r2 = r2 + 1
            r0[r14] = r2
            if (r2 == 0) goto L_0x00db
            r11 = r7
            goto L_0x00e0
        L_0x00db:
            int r14 = r14 + 1
            goto L_0x00cf
        L_0x00de:
            r11 = r25
        L_0x00e0:
            if (r11 != 0) goto L_0x00ef
        L_0x00e2:
            r1 = r0[r9]
            r2 = -1
            if (r1 != r2) goto L_0x00f2
            int[] r1 = cca_continue
            boolean r1 = com.cardinalcommerce.a.getSDKTransactionID.Cardinal(r0, r1)
            if (r1 == 0) goto L_0x00f2
        L_0x00ef:
            configure((int[]) r33)
        L_0x00f2:
            return
        */
        throw new UnsupportedOperationException("Method not decompiled: com.cardinalcommerce.a.protectKeys.init(int[], int[]):void");
    }

    /* JADX WARNING: Code restructure failed: missing block: B:16:0x0052, code lost:
        if (r0 == false) goto L_0x0054;
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */
    public static void init(int r13, int[] r14) {
        /*
            r0 = 0
            r1 = 1
            r2 = 0
            if (r13 == 0) goto L_0x003d
            long r4 = (long) r13
            r6 = 4294967295(0xffffffff, double:2.1219957905E-314)
            long r4 = r4 & r6
            r13 = r14[r0]
            long r8 = (long) r13
            long r8 = r8 & r6
            long r8 = r8 - r4
            long r8 = r8 + r2
            int r13 = (int) r8
            r14[r0] = r13
            r13 = 32
            long r8 = r8 >> r13
            int r10 = (r8 > r2 ? 1 : (r8 == r2 ? 0 : -1))
            if (r10 == 0) goto L_0x0030
            r10 = r14[r1]
            long r10 = (long) r10
            long r10 = r10 & r6
            long r8 = r8 + r10
            int r10 = (int) r8
            r14[r1] = r10
            long r8 = r8 >> r13
            r10 = 2
            r11 = r14[r10]
            long r11 = (long) r11
            long r11 = r11 & r6
            long r8 = r8 + r11
            int r11 = (int) r8
            r14[r10] = r11
            long r8 = r8 >> r13
        L_0x0030:
            r10 = 3
            r11 = r14[r10]
            long r11 = (long) r11
            long r6 = r6 & r11
            long r6 = r6 + r4
            long r8 = r8 + r6
            int r4 = (int) r8
            r14[r10] = r4
            long r4 = r8 >> r13
            goto L_0x003e
        L_0x003d:
            r4 = r2
        L_0x003e:
            int r13 = (r4 > r2 ? 1 : (r4 == r2 ? 0 : -1))
            if (r13 == 0) goto L_0x0054
            r13 = 4
        L_0x0043:
            r2 = 7
            if (r13 >= r2) goto L_0x0051
            r2 = r14[r13]
            int r2 = r2 + r1
            r14[r13] = r2
            if (r2 == 0) goto L_0x004e
            goto L_0x0052
        L_0x004e:
            int r13 = r13 + 1
            goto L_0x0043
        L_0x0051:
            r0 = r1
        L_0x0052:
            if (r0 != 0) goto L_0x0062
        L_0x0054:
            r13 = 6
            r13 = r14[r13]
            r0 = -1
            if (r13 != r0) goto L_0x0065
            int[] r13 = cca_continue
            boolean r13 = com.cardinalcommerce.a.getSDKTransactionID.Cardinal(r14, r13)
            if (r13 == 0) goto L_0x0065
        L_0x0062:
            configure((int[]) r14)
        L_0x0065:
            return
        */
        throw new UnsupportedOperationException("Method not decompiled: com.cardinalcommerce.a.protectKeys.init(int, int[]):void");
    }

    public static void configure(int[] iArr, int[] iArr2) {
        int[] iArr3 = new int[14];
        getSDKTransactionID.cca_continue(iArr, iArr3);
        init(iArr3, iArr2);
    }

    public static void Cardinal(int[] iArr, int i, int[] iArr2) {
        int[] iArr3 = new int[14];
        getSDKTransactionID.cca_continue(iArr, iArr3);
        while (true) {
            init(iArr3, iArr2);
            i--;
            if (i > 0) {
                getSDKTransactionID.cca_continue(iArr2, iArr3);
            } else {
                return;
            }
        }
    }

    public static void init(int[] iArr, int[] iArr2, int[] iArr3) {
        if (getSDKTransactionID.getSDKVersion(iArr, iArr2, iArr3) != 0) {
            long j = (((long) iArr3[0]) & 4294967295L) + 1;
            iArr3[0] = (int) j;
            long j2 = j >> 32;
            if (j2 != 0) {
                long j3 = j2 + (((long) iArr3[1]) & 4294967295L);
                iArr3[1] = (int) j3;
                long j4 = (j3 >> 32) + (((long) iArr3[2]) & 4294967295L);
                iArr3[2] = (int) j4;
                j2 = j4 >> 32;
            }
            long j5 = j2 + ((4294967295L & ((long) iArr3[3])) - 1);
            iArr3[3] = (int) j5;
            if ((j5 >> 32) != 0) {
                int i = 4;
                while (i < 7) {
                    int i2 = iArr3[i] - 1;
                    iArr3[i] = i2;
                    if (i2 == -1) {
                        i++;
                    } else {
                        return;
                    }
                }
            }
        }
    }

    private static void configure(int[] iArr) {
        long j = (((long) iArr[0]) & 4294967295L) - 1;
        iArr[0] = (int) j;
        long j2 = j >> 32;
        if (j2 != 0) {
            long j3 = j2 + (((long) iArr[1]) & 4294967295L);
            iArr[1] = (int) j3;
            long j4 = (j3 >> 32) + (((long) iArr[2]) & 4294967295L);
            iArr[2] = (int) j4;
            j2 = j4 >> 32;
        }
        long j5 = j2 + (4294967295L & ((long) iArr[3])) + 1;
        iArr[3] = (int) j5;
        if ((j5 >> 32) != 0) {
            int i = 4;
            while (i < 7) {
                int i2 = iArr[i] + 1;
                iArr[i] = i2;
                if (i2 == 0) {
                    i++;
                } else {
                    return;
                }
            }
        }
    }
}
