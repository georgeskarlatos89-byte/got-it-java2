...
public final class ValidateResponse extends getThreeDSRequestorAppURL implements isLocationDataConsentGiven {
    private final byte[] Cardinal;

    ValidateResponse(byte[] bArr) {
        this.Cardinal = bArr;
    }
    ...
    public final boolean configure(getThreeDSRequestorAppURL getthreedsrequestorappurl) {
        if (!(getthreedsrequestorappurl instanceof ValidateResponse)) {
            return false;
        }
        return getBackgroundColor.Cardinal(this.Cardinal, ((ValidateResponse) getthreedsrequestorappurl).Cardinal);
    }
