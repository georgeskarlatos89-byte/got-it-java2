...
public final class asFloatObj {
    static final int[] Cardinal = {-1, -1, 0, -1, -1, -1, -1, -2};
    private static int[] init = {1, 0, -2, 1, 1, -2, 0, 2, -2, -3, 3, -2, -1, -1, 0, -2};
    ...
    public static void cca_continue(int[] iArr, int[] iArr2) {
        if (getErrorMessage.configure(8, iArr, iArr2) != 0 || ((iArr2[7] >>> 1) >= Integer.MAX_VALUE && setErrorCode.cca_continue(iArr2, Cardinal))) {
            cca_continue(iArr2);
        }
    }

    public static void configure(int i, int[] iArr) {
        long j;
        if (i != 0) {
        ...
        if (j != 0 || ((iArr[7] >>> 1) >= Integer.MAX_VALUE && setErrorCode.cca_continue(iArr, Cardinal))) {
            cca_continue(iArr);
        }
    }

    public static void configure(int[] iArr, int[] iArr2, int[] iArr3) {
        if (setErrorCode.getInstance(iArr, iArr2, iArr3) != 0 || ((iArr3[7] >>> 1) >= Integer.MAX_VALUE && setErrorCode.cca_continue(iArr3, Cardinal))) {
            cca_continue(iArr3);
        }
    }

    public static void getInstance(int[] iArr, int[] iArr2) {
        if (getErrorMessage.cca_continue(8, iArr, 0, iArr2) != 0 || ((iArr2[7] >>> 1) >= Integer.MAX_VALUE && setErrorCode.cca_continue(iArr2, Cardinal))) {
            cca_continue(iArr2);
        }
    }

    public static int[] init(BigInteger bigInteger) {
        int[] Cardinal2 = setErrorCode.Cardinal(bigInteger);
        if ((Cardinal2[7] >>> 1) >= Integer.MAX_VALUE) {
            int[] iArr = Cardinal;
            if (setErrorCode.cca_continue(Cardinal2, iArr)) {
                setErrorCode.cleanup(iArr, Cardinal2);
            }
        }
        return Cardinal2;
    }
    ...
    public static void configure(int[] iArr, int[] iArr2) {
        ...
        if (z) {
            ...
            setErrorCode.getSDKVersion(Cardinal, iArr, iArr2);
        }
    }
    ...
    public static void cca_continue(int[] iArr, int[] iArr2, int[] iArr3) {
        if (setErrorCode.getSDKVersion(iArr, iArr2, iArr3) != 0) {
            ...
            iArr3[7] = (int) (j6 + ((4294967295L & ((long) iArr3[7])) - 1));
        }
    }
}
