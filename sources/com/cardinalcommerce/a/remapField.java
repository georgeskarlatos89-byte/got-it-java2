...
public final class remapField extends JSONNavi.init {
    public remapField(toJSONString tojsonstring, merge merge, merge merge2) {
        this(tojsonstring, merge, merge2, false);
    }

    public remapField(toJSONString tojsonstring, merge merge, merge merge2, boolean z) {
    ...
    remapField(toJSONString tojsonstring, merge merge, merge merge2, merge[] mergeArr, boolean z) {
    ...
    public final JSONNavi CardinalChallengeObserver() {
        ...
        return new remapField(tojsonstring, merge, merge2.cca_continue(merge3), mergeArr, this.Cardinal);
    }
    ...
    public final JSONNavi cca_continue(JSONNavi jSONNavi) {
        ...
        if (valueOf.onCReqSuccess()) {
            ...
            if (merge5.onCReqSuccess()) {
                return new remapField(configure, merge5, configure.cleanup().getWarnings(), this.Cardinal);
            }
            ...
            if (init3.onCReqSuccess()) {
                return new remapField(configure, init3, configure.cleanup().getWarnings(), this.Cardinal);
            }
        ...
        return new remapField(configure, merge5, merge4, new merge[]{merge6}, this.Cardinal);
    }
    ...
    public final JSONNavi configure(JSONNavi jSONNavi) {
        ...
        if (Cardinal.onCReqSuccess()) {
            return new remapField(configure, Cardinal, configure.cleanup().getWarnings(), this.Cardinal);
        }
        merge init2 = Cardinal.getInstance().init(init);
        merge init3 = Cardinal.init(instance5).init(instance4);
        return new remapField(configure, init2, Cardinal.cca_continue(instance5).getInstance().Cardinal(cca_continue, CardinalEnvironment.configure(), init3), new merge[]{init3}, this.Cardinal);
    }
    ...
    public final JSONNavi getActionCode() {
        ...
        if (cca_continue.onCReqSuccess()) {
            return new remapField(configure, cca_continue, configure.cleanup().getWarnings(), this.Cardinal);
        }
        ...
        return new remapField(configure, instance2, merge.configure(cca_continue, init).cca_continue(instance2).cca_continue(init2), new merge[]{init2}, this.Cardinal);
    }
    ...
    public final JSONNavi init() {
        return new remapField((toJSONString) null, onCReqSuccess(), getWarnings());
    }
}
