...
public final class JWSVerifier extends JSONNavi.init {
    public JWSVerifier(toJSONString tojsonstring, merge merge, merge merge2) {
        this(tojsonstring, merge, merge2, false);
    }

    public JWSVerifier(toJSONString tojsonstring, merge merge, merge merge2, boolean z) {
    ...
    JWSVerifier(toJSONString tojsonstring, merge merge, merge merge2, merge[] mergeArr, boolean z) {
    ...
    public final JSONNavi CardinalChallengeObserver() {
        ...
        return new JWSVerifier(tojsonstring, merge, merge2.cca_continue(merge3), mergeArr, this.Cardinal);
    }
    ...
    public final JSONNavi cca_continue(JSONNavi jSONNavi) {
        ...
        if (valueOf.onCReqSuccess()) {
            ...
            if (merge5.onCReqSuccess()) {
                return new JWSVerifier(configure, merge5, configure.cleanup(), this.Cardinal);
            }
            ...
            if (init3.onCReqSuccess()) {
                return new JWSVerifier(configure, init3, configure.cleanup(), this.Cardinal);
            }
        ...
        return new JWSVerifier(configure, merge5, merge4, new merge[]{merge6}, this.Cardinal);
    }
    ...
    public final JSONNavi configure(JSONNavi jSONNavi) {
        ...
        if (Cardinal.onCReqSuccess()) {
            return new JWSVerifier(configure, Cardinal, configure.cleanup(), this.Cardinal);
        }
        merge init2 = Cardinal.getInstance().init(init);
        merge init3 = Cardinal.init(instance5).init(instance4);
        return new JWSVerifier(configure, init2, Cardinal.cca_continue(instance5).getInstance().Cardinal(cca_continue, configure2, init3), new merge[]{init3}, this.Cardinal);
    }
    ...
    public final JSONNavi getActionCode() {
        ...
        if (cca_continue.onCReqSuccess()) {
            return new JWSVerifier(configure, cca_continue, configure.cleanup(), this.Cardinal);
        }
        ...
        return new JWSVerifier(configure, instance2, instance3.cca_continue(cca_continue).cca_continue(instance).init(instance3).cca_continue(merge3).cca_continue(instance2).cca_continue(init), new merge[]{init}, this.Cardinal);
    }
    ...
    public final JSONNavi init() {
        return new JWSVerifier((toJSONString) null, onCReqSuccess(), getWarnings());
    }
}
