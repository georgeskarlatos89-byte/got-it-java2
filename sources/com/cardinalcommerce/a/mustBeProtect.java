...
public final class mustBeProtect extends JSONNavi.init {
    public mustBeProtect(toJSONString tojsonstring, merge merge, merge merge2) {
        this(tojsonstring, merge, merge2, false);
    }

    public mustBeProtect(toJSONString tojsonstring, merge merge, merge merge2, boolean z) {
    ...
    mustBeProtect(toJSONString tojsonstring, merge merge, merge merge2, merge[] mergeArr, boolean z) {
    ...
    public final JSONNavi CardinalChallengeObserver() {
        ...
        return new mustBeProtect(tojsonstring, merge, merge2.cca_continue(merge3), mergeArr, this.Cardinal);
    }
    ...
    public final JSONNavi cca_continue(JSONNavi jSONNavi) {
        ...
        if (valueOf.onCReqSuccess()) {
            ...
            merge5 = instance2.getInstance().cca_continue(instance2).cca_continue(onValidated);
            if (merge5.onCReqSuccess()) {
                return new mustBeProtect(configure, merge5, configure.cleanup(), this.Cardinal);
            }
            ...
            if (init3.onCReqSuccess()) {
                return new mustBeProtect(configure, init3, configure.cleanup(), this.Cardinal);
            }
        ...
        return new mustBeProtect(configure, merge5, merge4, new merge[]{merge6}, this.Cardinal);
    }
    ...
    public final JSONNavi configure(JSONNavi jSONNavi) {
        ...
        if (Cardinal.onCReqSuccess()) {
            return new mustBeProtect(configure, Cardinal, configure.cleanup(), this.Cardinal);
        }
        merge init2 = Cardinal.getInstance().init(init);
        merge init3 = Cardinal.init(instance5).init(instance4);
        return new mustBeProtect(configure, init2, Cardinal.cca_continue(instance5).getInstance().Cardinal(cca_continue, configure2, init3), new merge[]{init3}, this.Cardinal);
    }
    ...
    public final JSONNavi getActionCode() {
        ...
        if (cca_continue.onCReqSuccess()) {
            return new mustBeProtect(configure, cca_continue, configure.cleanup(), this.Cardinal);
        }
        ...
        return new mustBeProtect(configure, instance2, instance3.cca_continue(cca_continue).cca_continue(instance).init(instance3).cca_continue(merge3).cca_continue(instance2).cca_continue(init), new merge[]{init}, this.Cardinal);
    }
    ...
    public final JSONNavi init() {
        return new mustBeProtect((toJSONString) null, onCReqSuccess(), getWarnings());
    }
}
