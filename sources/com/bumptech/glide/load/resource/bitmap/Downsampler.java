...
public final class Downsampler {
    ...
    private Bitmap decodeFromWrappedStreams(ImageReader imageReader, BitmapFactory.Options options, DownsampleStrategy downsampleStrategy, DecodeFormat decodeFormat, PreferredColorSpace preferredColorSpace, boolean z, int i, int i2, boolean z2, DecodeCallbacks decodeCallbacks) throws IOException {
        ...
        calculateScaling(imageType, imageReader, decodeCallbacks, this.bitmapPool, downsampleStrategy, exifOrientationDegrees, i8, i9, i3, i11, options);
        ...
        if (shouldUsePool(imageType2)) {
            if (i14 < 0 || i13 < 0 || !z2) {
                ...
                if (Log.isLoggable(TAG, 2)) {
                    Log.v(TAG, "Calculated target [" + i7 + "x" + i6 + "] for source [" + i14 + "x" + i13 + "], sampleSize: " + i16 + ", targetDensity: " + options3.inTargetDensity + ", density: " + options3.inDensity + ", density multiplier: " + f);
                }
            } else {
                i7 = i3;
                i6 = i11;
            ...
            if (i7 > 0 && i6 > 0) {
                setInBitmap(options3, this.bitmapPool, i7, i6);
            }
        }
        ...
        if (i11 > 0 && i12 > 0) {
            ...
            if (scaleFactor > 0.0f) {
                ...
                if (sampleSizeRounding != null) {
                    ...
                    if (Log.isLoggable(TAG, 2)) {
                        Log.v(TAG, "Calculate scaling, source: [" + i2 + "x" + i3 + "], degreesToRotate: " + i + ", target: [" + i13 + "x" + i14 + "], power of two scaled: [" + i9 + "x" + i10 + "], exact scale factor: " + scaleFactor + ", power of 2 sample size: " + max + ", adjusted scale factor: " + scaleFactor2 + ", target density: " + options2.inTargetDensity + ", density: " + options2.inDensity);
                        return;
                    }
                    return;
                }
            ...
            throw new IllegalArgumentException("Cannot scale with factor: " + scaleFactor + " from: " + downsampleStrategy2 + ", source: [" + i11 + "x" + i12 + "], target: [" + i13 + "x" + i14 + "]");
        } else if (Log.isLoggable(TAG, 3)) {
            Log.d(TAG, "Unable to determine dimensions for: " + imageType2 + " with target [" + i13 + "x" + i14 + "]");
        }
    }
