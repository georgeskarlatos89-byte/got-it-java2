...
public final class TreeRangeMap<K extends Comparable, V> implements RangeMap<K, V> {
    ...
    private void split(Cut<K> cut) {
        Map.Entry<K, V> lowerEntry = this.entriesByLowerBound.lowerEntry(cut);
        if (lowerEntry != null) {
    ...
    public void merge(Range<K> range, @CheckForNull V v, BiFunction<? super V, ? super V, ? extends V> biFunction) {
        ...
        if (!range.isEmpty()) {
            ...
            if (v != null) {
                ...
                for (Map.Entry<Cut<K>, RangeMapEntry<K, V>> value : entrySet) {
                    RangeMapEntry rangeMapEntry = (RangeMapEntry) value.getValue();
            ...
            while (it.hasNext()) {
                ...
                Object apply = biFunction.apply(((RangeMapEntry) next.getValue()).getValue(), v);
                if (apply == null) {
                    it.remove();
                } else {
    ...
    private class SubRangeMap implements RangeMap<K, V> {
        ...
        public void merge(Range<K> range, @CheckForNull V v, BiFunction<? super V, ? super V, ? extends V> biFunction) {
            Preconditions.checkArgument(this.subRange.encloses(range), "Cannot merge range %s into a subRangeMap(%s)", (Object) range, (Object) this.subRange);
