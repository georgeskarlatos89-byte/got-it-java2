...
public class PaymentsClient extends GoogleApi<Wallet.WalletOptions> {
    ...
    public Task<PaymentCardRecognitionIntentResponse> getPaymentCardRecognitionIntent(PaymentCardRecognitionIntentRequest paymentCardRecognitionIntentRequest) {
        return doRead(TaskApiCall.builder().run(new zzai(paymentCardRecognitionIntentRequest)).setFeatures(zzk.zzg).setAutoResolveMissingFeatures(true).setMethodKey(23716).build());
    }

    public Task<Boolean> isReadyToPay(IsReadyToPayRequest isReadyToPayRequest) {
        return doRead(TaskApiCall.builder().setMethodKey(23705).run(new zzah(isReadyToPayRequest)).build());
    }

    public Task<PaymentData> loadPaymentData(PaymentDataRequest paymentDataRequest) {
        return doWrite(TaskApiCall.builder().run(new zzag(paymentDataRequest)).setFeatures(zzk.zzc).setAutoResolveMissingFeatures(true).setMethodKey(23707).build());
    }
