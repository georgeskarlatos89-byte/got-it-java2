            ...
            while (!z) {
                ...
                while (!z && i < length) {
                    ...
                    if (isTokenSeparator(charAt) || isWhitespace(charAt)) {
                        ...
                        throw new ParseException(new StringBuffer("Invalid character before token (pos ").append(i).append("): ").append(this.currentHeader).toString());
                    }
                }
                if (!z) {
        ...
        if (i >= 0) {
            ...
            while (!z && i < length) {
                ...
                if (isTokenSeparator(charAt)) {
                    ...
                    throw new ParseException(new StringBuffer("Tokens without separator (pos ").append(i).append("): ").append(this.currentHeader).toString());
                } else {
                    throw new ParseException(new StringBuffer("Invalid character after token (pos ").append(i).append("): ").append(this.currentHeader).toString());
                }
            }
            return i;
        }
